            ...
            <div class="col-md-12 mt-3">
                                ...
                                <tbody class="text-md">
                                    <tr v-for="expense in expenses.data" v-bind:key="expense.expense_id">
                                        ...
                                        <td class="text-center">
                                            <button class="btn btn-primary btn-sm text-md" @click="editExpense(expense.expense_id,expense.expense_date,expense.expense_description,expense.expense_amount, expense.expense_category_id)"><font-awesome-icon icon="fa-solid fa-pen" /></button>
                                            <button class="btn btn-danger btn-sm text-md" @click="deleteExpense(expense.expense_id)"><font-awesome-icon icon="fa-solid fa-trash-can" /></button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div v-if="expenseId == -1 ? 0:1" class="card card-primary">
                    <div class="card-header">
                        ...
                        <button type="button" class="btn btn-tool" @click="closeUpdateExpenseBtn" ><font-awesome-icon icon="fa-solid fa-xmark" />
                        </button>
                        </div>
                    </div>
                    <!-- /.card-header -->
    ...
    export default {
        ...
        methods: {
            getAllExpenses: function (page = 1) {
                axios
                    .get(
                        `${process.env.VUE_APP_BACKEND_URL}/api/expense?page=` +
                        page +
                        "&paginate=" +
                        this.paginate +
                        "&search=" +
            ...
            getAllExpenseCategoriesForFilter: function () {
                axios
                    .get(`${process.env.VUE_APP_BACKEND_URL}/api/expensecategorieslist`)
                    .then(res => {
            ...
            getAmountForGivenDateAndCategory: function(){
                axios.get(`${process.env.VUE_APP_BACKEND_URL}/api/totalexpenseamount?fromdate=${this.fromDate}&todate=${this.toDate}&search=${this.search}&categoryid=${this.selectedExpenseCategoryForFilter.value}`).then((res) => {
            ...
            updateExpense: function () {
                if(this.validateExpenseDate() && this.validateExpenseDescription() && this.validateExpenseCategory() && this.validateExpenseAmount()){
                    ...
                    axios
                        .put(`${process.env.VUE_APP_BACKEND_URL}/api/expense`, payload)
                        .then(res => {
                            if(res.data.status == 1){
                                swal.fire({
                                    title: "Success",
            ...
            deleteExpense: function (expense_id) {
                axios
                    .delete(`${process.env.VUE_APP_BACKEND_URL}/api/expense/` + expense_id)
                    .then(res => {
                        if (res.data.status == 1) {
                            swal.fire({
                                title: "Success",
